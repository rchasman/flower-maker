...
export function FitnessBreakdown({ sessionId }: FitnessBreakdownProps) {
  ...
  if (!sessionId) {
    return (
      <div style={{ padding: '0.75rem', color: '#404040', fontSize: '0.6875rem' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
      ...
      {scores.map((score) => {
        ...
        return (
          <div key={score.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ fontSize: '0.625rem', color: '#737373', width: '4.5rem' }}>
              {env?.name ?? `Env #${score.environment_id}`}
            </span>
            <div style={{
              flex: 1, height: '0.375rem',
              background: '#1a1a1a', borderRadius: '0.1875rem',
              overflow: 'hidden',
            }}>
              <div style={{
                width: `${score.score}%`,
                ...
                background: score.score > 70 ? '#22c55e'
                  : score.score > 40 ? '#eab308'
                  : '#ef4444',
