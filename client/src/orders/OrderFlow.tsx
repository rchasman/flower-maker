...
export function OrderFlow({ session, onOrder }: OrderFlowProps) {
  const [orderPayload, setOrderPayload] = useState<Record<string, unknown> | null>(null)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      <div style={{
        ...
        <div style={{ color: '#a3a3a3', marginBottom: '0.5rem' }}>{session.prompt}</div>
      ...
      {Object.keys(fitnessScores).length > 0 && (
        <div style={{
          ...
          <div style={{ color: '#737373', marginBottom: '0.375rem', fontWeight: 500 }}>Fitness</div>
          {Object.entries(fitnessScores).map(([env, score]) => (
            <div key={env} style={{ display: 'flex', justifyContent: 'space-between', color: '#525252' }}>
              <span>{env}</span>
              <span style={{ color: score > 70 ? '#22c55e' : score > 40 ? '#eab308' : '#ef4444' }}>
                {score.toFixed(1)}
              </span>
            </div>
          ))}
...
function levelName(level: number): string {
  return ['', 'stem', 'group', 'bunch', 'arrangement', 'bouquet', 'centerpiece', 'installation'][level] ?? 'unknown'
}
