...
const EDITABLE_FIELDS: EditableField[] = [
  ...
  { path: 'personality.pollinator_attraction', label: 'Pollinator Attraction', type: 'number' },
  { path: 'personality.light_preference', label: 'Light', type: 'select', options: ['FullSun', 'PartialShade', 'FullShade', 'Nocturnal'] },
  { path: 'personality.wind_response', label: 'Wind Response', type: 'select', options: ['Rigid', 'Gentle', 'Dramatic', 'Dancing'] },
  ...
  { path: 'roots.mycorrhizal', label: 'Mycorrhizal', type: 'select', options: ['true', 'false'] },
]
...
export function PartEditor({ sessionId, specJson }: PartEditorProps) {
  ...
  const spec = (() => {
    try { return JSON.parse(specJson) as Record<string, unknown> }
    catch { return {} }
  })()
  ...
  const handleFork = () => {
    ...
    setOverrides({})
    console.log('[fork] Forked', Object.keys(overrides).length, 'parts for session', sessionId)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      {EDITABLE_FIELDS.map((field) => {
        ...
        const displayValue = overrideValue ?? String(currentValue ?? '')
            ...
            {field.type === 'select' && field.options ? (
              <select
                ...
                style={{
                  ...
                  borderRadius: '0.1875rem', color: '#e5e5e5',
                  fontSize: '0.6875rem',
                }}
              >
              ...
              <input
                ...
                style={{
                  ...
                  background: '#141414', border: '1px solid #262626',
                  borderRadius: '0.1875rem', color: '#e5e5e5',
                  fontSize: '0.6875rem', width: '4rem',
                }}
              />
            )}
            {overrideValue !== undefined && (
              <span style={{ color: '#eab308', fontSize: '0.5rem' }}>modified</span>
            )}
          </div>
        )
      })}
      ...
      {Object.keys(overrides).length > 0 && (
        ...
        >
          Fork {Object.keys(overrides).length} part{Object.keys(overrides).length > 1 ? 's' : ''}
        </button>
      )}

      {Object.keys(overrides).length === 0 && (
        <p style={{ color: '#404040', fontSize: '0.625rem', marginTop: '0.25rem' }}>
          Adjust values above to fork parts. Forks create personal variants that others can discover.
        </p>
      )}
    </div>
  )
