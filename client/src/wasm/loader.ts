...
export interface GardenSim {
  upsert_flower(session_id: number, spec_json: string, x: number, y: number): void
...
function createStub(): GardenSim {
  ...
  return {
    upsert_flower(sid: number, spec: string, x: number, y: number) { flowers.set(sid, { x, y, spec }) },
    wilt_flower(sid: number) { flowers.delete(sid) },
    remove_flower(sid: number) { flowers.delete(sid) },
    tick() { return flowers.size },
    get_merge_events() { return '[]' },
    render_data() {
      const data = [...flowers.entries()].map(([sid, f]) => ({
        sid, x: f.x, y: f.y, rotation: 0, scale: 1, alpha: 1,
        spec_name: 'stub', has_aura: false, has_glow: false, particles: 0,
      }))
      return JSON.stringify(data)
    },
    flower_count() { return flowers.size },
    wind_x() { return 0 },
    wind_y() { return 0 },
    ambient_light() { return 1 },
    time_of_day() { return 12 },
  }
}
