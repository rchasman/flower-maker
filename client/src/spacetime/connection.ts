...
const SPACETIMEDB_MODULE = import.meta.env.VITE_SPACETIMEDB_MODULE ?? 'flower-maker'
...
export type ConnectionListener = (state: ConnectionState, conn: DbConnection | null) => void
...
export async function connect(): Promise<DbConnection> {
  ...
  try {
    ...
    console.warn('[spacetimedb] bindings not generated yet — run `spacetime generate`', err)
    notify('error', null)
    throw err
  }
}
