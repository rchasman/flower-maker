  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        <h1 style={{ fontSize: '1.25rem', fontWeight: 400, letterSpacing: '-0.02em' }}>
          flower-maker
        </h1>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', fontSize: '0.8125rem', color: '#737373' }}>
      ...
      <div style={{
        ...
        {state !== 'connected' && (
          <div style={{
            ...
            {state === 'connecting' ? 'Connecting to SpacetimeDB...' : 'Not connected — start SpacetimeDB and refresh'}
          </div>
        )}
      </div>
    </div>
...
function ConnectionDot({ state }: { state: string }) {
  const color = state === 'connected' ? '#22c55e'
    : state === 'connecting' ? '#eab308'
    : '#ef4444'
  return (
    <span style={{
      width: 8, height: 8,
