...
const AdornmentSpecSchema = z.object({
  ...
  mood: z
    .string()
    .describe("rustic, elegant, wild, minimal, lush, or dramatic"),
  evolved_from: z
    .string()
    .optional()
    .describe(
      "Description of what this adornment evolved from, if merging existing arrangements",
    ),
});
...
const ArrangementSchema = z.object({
  ...
  adornment_spec: AdornmentSpecSchema.describe(
    "Structured adornment specification for rendering",
  ),
  harmony_note: z
    .string()
...
export async function handleCombine(request: Request) {
  ...
  const inheritanceContext =
    body.parent_adornments && body.parent_adornments.length > 0
